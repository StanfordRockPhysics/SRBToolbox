function [ss,ttu]=quickseis(vel,dens,varargin);
%[ss,ttu]=quickseis(vel,dens,dx,dz,top,freq,snr);
%
...
%
% See also: KENNET, KENFRTT, PSPEC2DSH, BORN2D, BORNFILT, EZSEIS, EZSEIS2
...
case 0, answer=inputdlg(prompt, 'SRB QuickSeis', 1, def);
...
fnn = 4.*freq;   % To minimize dead band, choose Nyquist only twice cutoff frequency
R = floor(fn/fnn);
if R>1, 
...
seisrwb(ss,min(ttu),R*(ttu(2)-ttu(1)),0,dx,0);
