function varargout=flsubk(velp1,vels1,poro,dens)
% [velp2,vels2,ss1,ss2,ttu]=flsubk(velp1,vels1,poro,dens)
%
...
% Fluid and mineral properties, and other parameters
...
% axis given by TTU vector. Optional input DENS (scalar) is the density in
% the zone outside the region of interest. Default=2200 kg/m3. This is
% required for seismogram calculation.
%
% Fluid substitution is done using Gassmann's equations, with the uniform 
% effective fluid (Reuss average) saturation model.
% Fluid property inputs are in terms of bulk moduli and density.
% Normal incidence seismograms are calculated from reflectivity and
% horizontal Fresnel zone calculations (see EZSEIS).
%
% See also FLSUB, FLPROPUI, GASSMNV, GASSMNK, EZSEIS, EZSEIS2
...
defans={2650,36,800.0,1.0,1000.0,2.2};
for k=1:length(defans),defans{k}=num2str(defans{k});end;
...
prmt={'Mineral density (kg/m3)', ...
      ...
      'Fluid density initial (kg/m3)', ...
      'Fluid bulk modulus initial (GPa)', ...
      'Fluid density final (kg/m3)', ...
      'Fluid bulk modulus final (GPa)'};
...
break
 otherwise
flinpt=inputdlg(prmt,'Mineral & Fluids',1,defans);
defans=flinpt; button=prod(size(flinpt));

if button ~= 0
ro0=str2num(flinpt{1}); k0=str2num(flinpt{2}).*1e9;
rofl1=str2num(flinpt{3}); kfl1=str2num(flinpt{4});
rofl2=str2num(flinpt{5}); kfl2=str2num(flinpt{6});

kfl1=kfl1.*1e9; kfl2=kfl2.*1e9; 
