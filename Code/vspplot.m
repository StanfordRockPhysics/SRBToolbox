function vspplot(x,t0,dt,nd,sf)
% VSPPLOT(X,T0,DT,ND,SF) 
% VSP wiggle plot with filled positive area
...
% Time axis is horizontal (x) and positive towards right.
% Depth axis is vertical.
...
%  y(:,i)=y(:,i)+(i-1.)*0.5;    % for surface seismic and VSP
%  zw0(:,i)=zw0(:,i)+(i-1.)*0.5; % for surface seismic and VSP
  y=y-scale(ones(nr,1),:);    % for surface seismic and VSP
  zw0=zw0-scale(ones(nr+2,1),:); % for surface seismic and VSP
...
fill(tz,zw0,'k'), hold on
plot(ty,y,'-k'), hold off
xlabel('time'); %set(gca, 'ydir', 'reverse');

%fill(zw0,tz,'w'), hold on % for surface seismic
%plot(y,ty,'-w'), hold off % for surface seismic
%set(gca,'ydir','reverse') % for surface seismic
%ylabel('time'); % for surface seismic
